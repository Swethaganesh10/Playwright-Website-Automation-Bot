...
#  Use environment variable with fallback to Server 1
...
# NH patterns for quote (NHHQ) and policy (NHH) numbers
QUOTE_REGEX = re.compile(r"\bNHHQ\w+\b", re.I)
NH_POLICY_REGEX = re.compile(r"\bNHH\w+\b", re.I)
...
def normalize_state(s):
    ...
    s = (s or "").strip()
    return m.get(s.upper(), s or "New Hampshire")
...
def dismiss_backdrop(page, timeout=4000):  # OPTIMIZED: Reduced from 8000 to 4000
    try:
...
# ---------- login ----------
def login(page):
    # Force reload .env file (clears any cached credentials)
    load_dotenv(override=True)  # CRITICAL: override=True forces fresh read
...
# ---------- new quote ----------
def create_new_quote(page, state="New Hampshire", effective_date="", line_of_business="HO3"):
    ...
    # NEW: Select line of business radio button
...
def fill_applicant_info(page, applicant):
    ...
    #  Keyboard navigation for address (same as MA & CT)
...
# ---------- branch & agency (NH - similar to CT, no separate producer) ----------
...
def quote_and_issue(page, applicant):
    ...
    # OPTIMIZED: Faster producer click check
    try:
        prod = page.locator("#producerNewCoLiteral")
        if prod.is_visible(timeout=800):  # OPTIMIZED: From 1500 to 800
            safe_action(lambda: prod.click(), "Producer click")
    except Exception:
        pass

    # OPTIMIZED: Faster popup check
    try:
        page.get_by_text("x New Quote").click(timeout=800)  # OPTIMIZED: From 1500 to 800
    ...
    quote_btn.wait_for(state="visible", timeout=5000)  # OPTIMIZED: From 15000 to 5000
    ...
    # CHANGE #4: Check for DTQ/DTB popup on MAIN PAGE (only if VISIBLE)
    try:
    ...
    # CHANGE #1: Check for CRN/FCRA decline popup INSIDE iframe
    try:
    ...
    # NO cracks question for NH (CT only)
    ...
    # --- Producer License Number (NH-specific - REQUIRED) ---
    try:
        producer_row = target.get_by_role("row", name=re.compile(r"Producer License Number", re.I))
        producer_row.locator("a").click()
        log("Expanded producer license dropdown for NH")
        
        time.sleep(1)
        
        # Try known NH producers
        producer_selected = False
        nh_producers = ["Marley, Bob", "SMITH, MICHAEL", "Marley", "Smith"]
        
        for producer_name in nh_producers:
            try:
                ...
                if producer_option.count() > 0:
                    ...
                    log(f"✓ Selected NH producer: {producer_name}")
            ...
            try:
                ...
                for i, option in enumerate(all_options[:5]):
                    try:
                        ...
                        option.click()
                        log(f"✓ Selected NH producer: {option_text}")
                        producer_selected = True
                        break
                    except:
                        continue
        ...
        if not producer_selected:
            try:
                ...
                """)
                if result:
                    log(f"✓ Selected NH producer via JS: {result}")
            except Exception as js_error:
                log(f"[ERROR] JavaScript fallback failed: {js_error}")
                    
    except Exception as e:
        log(f"[ERROR] NH Producer License Number selection failed: {e}")
        raise RuntimeError(f"Producer License Number required for NH but selection failed: {e}")
...
def quote_only(page, applicant):
    ...
    # Handle Exit popup on MAIN PAGE (appears after navigation)
    time.sleep(1)
    try:
