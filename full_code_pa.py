...
# PA patterns for quote (PAHQ) and policy (PAH) numbers
QUOTE_REGEX = re.compile(r"\bPAHQ\w+\b", re.I)
PA_POLICY_REGEX = re.compile(r"\bPAH\w+\b", re.I)
...
def normalize_state(s):
    ...
    s = (s or "").strip()
    return m.get(s.upper(), s or "Pennsylvania")
...
def dismiss_backdrop(page, timeout=4000):  # OPTIMIZED
    try:
...
# ---------- new quote ----------
def create_new_quote(page, state="Pennsylvania", effective_date="", line_of_business="HO3"):
...
# ---------- branch, agency, and producer (PA specific) ----------
def select_branch_and_agency(
    ...
    log("Select branch, agency, and producer (PA)")

    # --- Branch ---
    br_row = page.get_by_role("row", name=re.compile(r"^Branch", re.I))
    br_row.locator("a").click()
    time.sleep(0.5)

    # Use branch_filter parameter instead of hardcoded
    try:
        page.locator("#branchNewCo_chosen").get_by_text(
            re.compile(re.escape(branch_filter), re.I)
        ).click()
        log(f"Branch selected: {branch_name}")
    except Exception as e:
        log(f"[WARN] Primary branch selection failed: {e}, trying hardcoded Palisades")
        page.locator("#branchNewCo_chosen").get_by_text(re.compile(r"Palisades", re.I)).click()
        log("Branch selected: Palisades (fallback)")

    time.sleep(1)

    # --- Agency with SAFETY NET (UI first, hardcoded fallback) ---
    ag_row = page.get_by_role("row", name=re.compile(r"^Agency", re.I))
    
    # Hardcoded fallback value
    HARDCODED_AGENCY = "A.H. Meyers"
    
    # Determine primary agency (from UI)
    primary_agency = agency_filter if agency_filter and agency_filter.lower() != "a.h. meyers" else None
    
    try:
        # Wait for dropdown to populate (30 seconds - CRITICAL!)
        ...
        # STEP 1: Try UI value if provided
        if primary_agency:
            log(f"Attempting UI agency selection: {primary_agency}")
            
            # Try selecting UI value with all methods
            try:
                ag_row.locator("a").click()
                time.sleep(0.5)
                page.locator("#agencyNewCo_chosen").get_by_text(
                    re.compile(re.escape(primary_agency), re.I)
                ).click(timeout=3000)
                log(f"✓ Agency selected (UI value): {agency_name}")
                agency_selected = True
            except:
                log(f"[INFO] UI click failed, trying search...")
                try:
                    search_box = page.locator("#agencyNewCo_chosen input[type='text']")
                    search_box.fill(primary_agency, timeout=2000)
                    time.sleep(0.3)
                    page.locator(
                        "#agencyNewCo_chosen .chosen-results li.active-result"
                    ).first.click(timeout=2000)
                    log(f"✓ Agency selected (UI search): {agency_name}")
                    agency_selected = True
                except:
                    log(f"[INFO] UI search failed")
        
        # STEP 2: Safety Net - Try hardcoded if UI failed
        if not agency_selected:
            log(f"[WARN] UI value failed, trying hardcoded fallback: {HARDCODED_AGENCY}")
            
            # Method 1: JavaScript with hardcoded patterns
            try:
                result = page.evaluate(
                    """() => {
                        const el = document.querySelector('#agencyNewCo');
                        if (!el || el.options.length <= 1) return 'NO_OPTIONS';
                        
                        const patterns = [
                            'A.H. Meyers and Company, INC',
                            'A.H. Meyers and Company',
                            'A.H. Meyers',
                            'Meyers and Company'
                        ];
                        ...
                        for (const pattern of patterns) {
                            for (let i = 0; i < el.options.length; i++) {
                                if (el.options[i].text.includes(pattern)) {
                                    el.selectedIndex = i;
                                    const selectedText = el.options[i].text;
                ...
                if result and result not in ['NO_OPTIONS', 'NOT_FOUND']:
                    log(f"✓ Agency selected (hardcoded JavaScript): {result}")
                    agency_selected = True
            except:
                log("[INFO] Hardcoded JavaScript failed")
            
            # Method 2: UI Click with hardcoded
            if not agency_selected:
                try:
                    ag_row.locator("a").click()
                    time.sleep(0.5)
                    page.locator("#agencyNewCo_chosen").get_by_text(
                        re.compile(r"A\.H\.\s*Meyers.*Company.*INC", re.I)
                    ).click(timeout=3000)
                    log("✓ Agency selected (hardcoded UI click)")
                    agency_selected = True
                except:
                    log("[INFO] Hardcoded UI click failed")
            
            # Method 3: Search with hardcoded
            if not agency_selected:
                try:
                    search_box = page.locator("#agencyNewCo_chosen input[type='text']")
                    search_box.fill(HARDCODED_AGENCY, timeout=2000)
                    time.sleep(0.3)
                    page.locator(
                        "#agencyNewCo_chosen .chosen-results li.active-result"
                    ).first.click(timeout=2000)
                    log("✓ Agency selected (hardcoded search)")
                    agency_selected = True
                except:
                    log("[INFO] Hardcoded search failed")
        
        if not agency_selected:
            raise RuntimeError("Agency selection failed with both UI value and hardcoded fallback!")
                    
    except Exception as e:
        log(f"[ERROR] Agency selection completely failed: {e}")
        raise RuntimeError(f"Could not select agency: {e}")

    time.sleep(1)

    # --- Producer with SAFETY NET ---
    HARDCODED_PRODUCER = "A.H. Meyers"
    primary_producer = producer_filter if producer_filter and producer_filter.lower() != "a.h. meyers" else None
    
    try:
        page.get_by_role("cell", name="--Select--").locator("a").click()
        time.sleep(0.5)
        ...
        # STEP 1: Try UI value
        if primary_producer:
            log(f"Attempting UI producer selection: {primary_producer}")
            try:
                page.locator("#producerNewCo_chosen").get_by_text(
                    re.compile(re.escape(primary_producer), re.I)
                ).click()
                log(f"✓ Producer selected (UI value): {producer_name}")
                producer_selected = True
            except:
                log(f"[INFO] UI producer selection failed")
        
        # STEP 2: Try hardcoded fallback
        if not producer_selected:
            log(f"Trying hardcoded producer: {HARDCODED_PRODUCER}")
            try:
                page.locator("#producerNewCo_chosen").get_by_text(
                    re.compile(r"A\.H\.\s*Meyers.*3018200", re.I)
                ).click()
                log("✓ Producer selected (hardcoded): A.H. Meyers and Company - 3018200")
                producer_selected = True
            except:
                # JavaScript fallback
                page.evaluate(
                ...
                log("✓ Producer selected (JavaScript fallback)")
                producer_selected = True
        
        # Close dropdown if needed (for MA/PA compatibility)
        if not producer_selected:
            try:
                ...
                pass
            log("[INFO] Producer will be auto-selected")
...
def quote_and_issue(page, applicant):
    ...
    quote_btn.wait_for(state="visible", timeout=5000)  # OPTIMIZED
    ...
    # Check for DTQ/DTB popup on MAIN PAGE (only if VISIBLE)
    try:
    ...
    # Check for CRN decline popup INSIDE iframe
    try:
        fcra_close_btn = frame.locator("#fcra-dialog-close")
        fcra_close_btn.wait_for(state="visible", timeout=1500)
        ...
        # time.sleep(1)
        try:
            page.locator("#logoSection").get_by_role("button").click(timeout=2000)
            log("Clicked logo button to go home")
        except:
            try:
                page.locator("#logoSection a").first.click(timeout=2000)
                log("Clicked logo link to go home")
            except:
                log("[WARN] Could not click logo, will try goto")
                page.goto(HOME_URL, timeout=7000)
    ...
    # --- Producer License Number (PA-specific - REQUIRED) ---
    try:
        producer_row = target.get_by_role("row", name=re.compile(r"Producer License Number", re.I))
        producer_row.locator("a").click()
        log("Expanded producer license dropdown for PA")
        
        time.sleep(1)
        
        producer_selected = False
        pa_producers = ["The Approved Bob Strohmenger Both", "Strohmenger", "Bob"]
        
        for producer_name in pa_producers:
            try:
                producer_option = target.locator("#producerLicenseNumber_chosen").get_by_text(
                    re.compile(re.escape(producer_name), re.I)
                )
                if producer_option.count() > 0:
                    producer_option.first.click()
                    log(f"✓ Selected PA producer: {producer_name}")
                    producer_selected = True
                    break
            except:
                continue
            ...
            try:
                ...
                for i, option in enumerate(all_options[:5]):
                    try:
                        ...
                        option.click()
                        log(f"✓ Selected PA producer: {option_text}")
                        producer_selected = True
                        break
                    except:
                        continue
        ...
        if not producer_selected:
            try:
                ...
                """)
                if result:
                    log(f"✓ Selected PA producer via JS: {result}")
            except Exception as js_error:
                log(f"[ERROR] JavaScript fallback failed: {js_error}")
                    
    except Exception as e:
        log(f"[ERROR] PA Producer License Number selection failed: {e}")
        raise RuntimeError(f"Producer License Number required for PA but selection failed: {e}")
