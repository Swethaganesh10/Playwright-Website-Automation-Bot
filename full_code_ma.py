...
# MA patterns for quote (MAHQ) and policy (MAH) numbers
QUOTE_REGEX = re.compile(r"\bMAHQ\w+\b", re.I)
MA_POLICY_REGEX = re.compile(r"\bMAH\w+\b", re.I)
...
def normalize_state(s):
    ...
    s = (s or "").strip()
    return m.get(s.upper(), s or "Massachusetts")
...
# ---------- new quote ----------
def create_new_quote(page, state="Massachusetts", effective_date="", line_of_business="HO3"):
...
def fill_applicant_info(page, applicant):
    ...
    safe_action(lambda: addr_box.click(), "Click again")
    ...
    #  Arrow Down + Enter (bypasses all viewport issues!)
    try:
        log("[DEBUG] Pressing Arrow Down to select first suggestion...")
        safe_action(lambda: addr_box.press("ArrowDown"), "Press Arrow Down")
        
        time.sleep(0.3)
        
        log("[DEBUG] Pressing Enter to confirm selection...")
        safe_action(lambda: addr_box.press("Enter"), "Press Enter")
        
        log("✓ Selected address using keyboard navigation")
        ...
        log(f"[WARN] Keyboard selection failed: {e}")
        addr_box.press("Enter")
        
        
# ---------- branch, agency & producer (MA specific) ----------
def select_branch_and_agency(
    ...
    log("Select branch, agency & producer (MA)")
    ...
    try:
        ...
        # NEW: Wait for page to settle after agency selection
        time.sleep(1)
        ...
        if producer_name:
            try:
                ...
                try:
                    prod_row.locator(".chosen-container input").fill(producer_filter)
                except:
                    pass
...
def quote_and_issue(page, applicant):
    ...
    # CHANGE #1: Check for CRN decline popup INSIDE iframe
    try:
    ...
    # NO cracks question for MA (CT only)
        ...
        if producer_row.count() > 0:
            ...
            for i, option in enumerate(all_options[:5]):
                try:
                    ...
                    option.click()
                    log(f"✓ Selected MA producer: {option_text}")
                    break
                except:
                    continue
        else:
            log("[INFO] No Producer License Number field (expected for MA)")
    except Exception as e:
        log(f"[INFO] Producer License Number not found: {e}")
