...
# CT patterns for quote (CTHQ) and policy (CTH) numbers
QUOTE_REGEX = re.compile(r"\bCTHQ\w+\b", re.I)
CT_POLICY_REGEX = re.compile(r"\bCTH\w+\b", re.I)
...
def normalize_state(s):
    ...
    s = (s or "").strip()
    return m.get(s.upper(), s or "Connecticut")
...
def dismiss_backdrop(page, timeout=4000):  # OPTIMIZED: Reduced from 5000 to 4000
    try:
...
# ---------- new quote ----------
def create_new_quote(page, state="Connecticut", effective_date="", line_of_business="HO3"):
    ...
    try:
        ...
        if producer_name:
            try:
                ...
                time.sleep(1)  # Wait for producer dropdown to populate
                ...
                time.sleep(0.5)
                # Try to select the specified producer
                producer_option = (
                    page.locator(".chosen-drop .chosen-results li")
                    .filter(has_text=re.compile(re.escape(producer_name), re.I))
                    .first
                )
                producer_option.wait_for(state="visible", timeout=10000)
...
def quote_and_issue(page, applicant):
    ...
    # Check for DTQ/DTB popup on MAIN PAGE
    try:
    ...
    try:
        frame.evaluate("() => { window.scrollTo(0, 100); window.scrollTo(0, 0); }")
    except:
        pass
    ...
    try:
        frame.locator("#cracksInFoundationYesNoDiv").get_by_text("No").click(timeout=6000)
        log("✓ Answered 'No' to cracks in foundation (CT specific)")
        time.sleep(2)
    except Exception:
        log("[INFO] Cracks question not present (skipping)")
        ...
        try:
            ...
            for i, option in enumerate(all_options[:5]):
                try:
                    ...
                    if "select" in option_text.lower() or option_text == "--" or not option_text:
                        log(f"  Skipping placeholder option: '{option_text}'")
                        continue
                    
                    option.click()
                    log(f"✓ Selected CT producer: {option_text}")
                    producer_selected = True
                    break
                except Exception as opt_error:
                    log(f"  Could not select option {i}: {opt_error}")
                    continue
            
            if not producer_selected:
                raise Exception("No real producer options found")
                
        except Exception as e:
            log(f"Primary method failed: {e}, trying fallback")
            
            ct_producers = ["JONES, MARK"]
            
            for producer_name in ct_producers:
                try:
                    option = target.locator("#producerLicenseNumber_chosen").get_by_text(
                        producer_name, exact=False
                    )
                    if option.count() > 0:
                        option.first.click()
                        log(f"✓ Selected hardcoded CT producer: {producer_name}")
                        producer_selected = True
                        break
                except:
                    continue
            ...
            if not producer_selected:
                try:
                    ...
                    """)
                    if result:
                        log(f"✓ Selected CT producer via JS: {result}")
                except Exception as js_error:
                    log(f"[ERROR] JavaScript fallback failed: {js_error}")
                    
    except Exception as e:
        log(f"[ERROR] CT Producer License Number selection failed: {e}")
        raise RuntimeError(f"Producer License Number required for CT but selection failed: {e}")
