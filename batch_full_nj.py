...
import os
# IMPORT THE NEW JERSEY CORE MODULE
import full_code_nj as m
import argparse
...
# NJ specific defaults
NJ_BRANCH_FILTER = "Palisades"
NJ_BRANCH_NAME = "Palisades"  
NJ_AGENCY_FILTER = "A & L Insurance"
NJ_AGENCY_NAME = "A & L Insurance Agency, LLC"
...
def main(
    in_csv="input_nj.csv", 
    out_csv="output_nj.csv", 
    out_xlsx=None, 
    headless=False,
    quotes_only=False 
):
    print("[INFO] New Jersey Complete Processing Script (Quote + Policy)")
    print(f"[INFO] Using NJ-specific core module with policy issuance")
        ...
        try:
            ...
            with (
                ...
                for i, row in enumerate(reader, start=1):
                    ...
                    # NJ specific configuration
                    load_dotenv(override=True)
                    
                    state = "New Jersey"
                    
                    # Read from .env, fallback to CSV, then hardcoded defaults
                    env_branch = os.getenv("PR_NJ_BRANCH", "").strip()
                    env_agency = os.getenv("PR_NJ_AGENCY", "").strip()
                    env_producer = os.getenv("PR_NJ_PRODUCER", "").strip()
                    
                    branch_name = env_branch or row.get("branch_name") or NJ_BRANCH_NAME
                    agency_name = env_agency or row.get("agency_name") or NJ_AGENCY_NAME
    ...
    print("\n" + "=" * 60)
    print("NEW JERSEY COMPLETE PROCESSING FINISHED")
...
if __name__ == "__main__":
    in_csv = sys.argv[1] if len(sys.argv) > 1 else "input_nj.csv"
    out_csv = (
        sys.argv[2]
        if len(sys.argv) > 2
        else f"output_nj_{datetime.now().strftime('%Y%m%d_%H%M%S')}.csv"
    )
