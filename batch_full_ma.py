...
# IMPORT THE MASSACHUSETTS CORE MODULE
import full_code_ma as m
...
# MA specific defaults
MA_BRANCH_FILTER = "New England"
MA_BRANCH_NAME = "BHIC0035 | New England"  
MA_AGENCY_FILTER = "columbia"
MA_AGENCY_NAME = "Columbia Insurance Agency"
MA_PRODUCER_FILTER = "columbia"
MA_PRODUCER_NAME = "Columbia Insurance Agency, Inc. - 0001001"
...
def normalize_address(addr: str) -> str:
    ...
    # Example: "235 Sheridan St North Easton MA 02356"
    match = re.match(
    ...
    if match2:
        ...
        # Add comma after street
        return f"{street}, {rest}"
    
    # No street type found, return as-is
    return addr


def main(
    in_csv="input_ma.csv", 
    out_csv="output_ma.csv", 
    out_xlsx=None, 
    headless=False,
    quotes_only=False 
):
    print("[INFO] Massachusetts Complete Processing Script (Quote + Policy)")
    print(f"[INFO] Using MA-specific core module with policy issuance")
        ...
        try:
            ...
            with (
                ...
                for i, row in enumerate(reader, start=1):
                    ...
                    # MA specific configuration - read from .env first
                    load_dotenv(override=True)

                    state = "Massachusetts"

                    # Read from .env, fallback to CSV, then hardcoded defaults
                    env_branch = os.getenv("PR_MA_BRANCH", "").strip()
                    env_agency = os.getenv("PR_MA_AGENCY", "").strip()
                    env_producer = os.getenv("PR_MA_PRODUCER", "").strip()

                    branch_name = env_branch or row.get("branch_name") or MA_BRANCH_NAME
                    agency_name = env_agency or row.get("agency_name") or MA_AGENCY_NAME
    ...
    print("\n" + "=" * 60)
    print("MASSACHUSETTS COMPLETE PROCESSING FINISHED")
...
if __name__ == "__main__":
    in_csv = sys.argv[1] if len(sys.argv) > 1 else "input_ma.csv"
    out_csv = (
        sys.argv[2]
        if len(sys.argv) > 2
        else f"output_ma_{datetime.now().strftime('%Y%m%d_%H%M%S')}.csv"
    )
