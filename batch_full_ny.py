...
import os
# IMPORT THE NEW YORK CORE MODULE
import full_code_ny as m
...
# NY specific defaults
NY_BRANCH_FILTER = "Palisades"
NY_BRANCH_NAME = "Palisades"  
NY_AGENCY_FILTER = "A.H. Meyers"
NY_AGENCY_NAME = "A.H. Meyers and Company"
NY_PRODUCER_FILTER = "A.H. Meyers"
NY_PRODUCER_NAME = "A.H. Meyers and Company"
...
def main(
    in_csv="input_ny.csv", 
    out_csv="output_ny.csv", 
    out_xlsx=None, 
    headless=False,
    quotes_only=False 
):
    print("[INFO] New York Complete Processing Script (Quote + Policy)")
    print(f"[INFO] Using NY-specific core module with policy issuance")
        ...
        try:
            ...
            with (
                ...
                for i, row in enumerate(reader, start=1):
                    ...
                    env_branch = os.getenv("PR_NY_BRANCH", "").strip()
                    env_agency = os.getenv("PR_NY_AGENCY", "").strip()
                    env_producer = os.getenv("PR_NY_PRODUCER", "").strip()
                    
                    branch_name = env_branch or row.get("branch_name") or NY_BRANCH_NAME
                    agency_name = env_agency or row.get("agency_name") or NY_AGENCY_NAME
                    producer_name = env_producer or row.get("producer_name") or NY_PRODUCER_NAME
    ...
    print("\n" + "=" * 60)
    print("NEW YORK COMPLETE PROCESSING FINISHED")
...
if __name__ == "__main__":
    in_csv = sys.argv[1] if len(sys.argv) > 1 else "input_ny.csv"
    out_csv = (
        sys.argv[2]
        if len(sys.argv) > 2
        else f"output_ny_{datetime.now().strftime('%Y%m%d_%H%M%S')}.csv"
    )
